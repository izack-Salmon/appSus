...
export default {
    template: `
        ...
        <email-sideRuler />
        <div>
            <email-filter @filterRead="setFilterRead"  @filtered="setFilter" />
    ...
    created() {
        ...
        eventBus.$on('new email created', (newEmail) => {
            return emailService.add(newEmail).then(() => {
                this.loadEmails();
            });
        });
        ...
        eventBus.$on('set isRead to true', (id) => {
            ...
            emailService.getById(id).then((email) => {
                console.log(email);
                email.isRead = true;
                const msg = { type: 'success', txt: 'Mark as read' };
                eventBus.$emit('showMsg', msg);
    ...
    methods: {
        ...
        setFilterRead(Boolean) {
            // console.log(Boolean);
            this.criteria.isRead = Boolean;
            this.loadEmails();
        },
    },
    computed: {
        ...
        emailCriteriaToShow() {
            if (!this.criteria) {
                ...
                const emailsToShow = this.emails.filter((email) => {
                    return (
                        email.txt.toLowerCase().includes(searchStr) ||
                        (email.subject.toLowerCase().includes(searchStr) && email.status.includes(status))
                    );
                });
                return emailsToShow;
            }
